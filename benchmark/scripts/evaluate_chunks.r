...
plot_per_thread_count <- function( timing, thread_key = "1_t", title, yl = c(2, 22) ) {
    ...
    print( "SD (500 to 5000 sequences) (1. Rfam, 2. Uniprot): " )
    data_r = data[1,-c(1,2,9)]
    data_u = data[2,-c(1,2,9)]
...
par( mfrow = c(2, 1) )
plot_per_thread_count( timing, "1_t", "Average runtime using 1 thread", c(1, 20) )
plot_per_thread_count( timing, "8_t", "Average runtime using 8 threads", c(0.3, 5) )
dev.off()
