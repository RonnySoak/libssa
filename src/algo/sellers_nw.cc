...
void nw(char * dseq, char * dend, char * qseq, char * qend, long * score_matrix,
        unsigned long gapopen, unsigned long gapextend, unsigned long * nwscore,
        unsigned long * nwdiff, unsigned long * nwalignmentlength,
        char ** nwalignment, unsigned char * directions, unsigned long * hearray,
        unsigned long queryno, unsigned long dbseqno) {
    ...
    // TODO code for Sellers NW !!!!!111!!!!!1
    ...
    for (j = 0; j < dlen; j++) {
        ...
        for (i = 0; i < qlen; i++) {
            ...
            h = MIN(h, f);
            h = MIN(h, e);
            directions[index] |= (e == h ? maskleft : 0);
            ...
            *hep = h;
            ...
            directions[index] |= (f < h ? maskextup : 0);
            directions[index] |= (e < h ? maskextleft : 0);
            f = MIN(h, f);
            e = MIN(h, e);
    ...
    char * cigar = xmalloc(qlen + dlen + 1);
