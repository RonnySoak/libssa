...
void dprofile_fill16(int16_t * dprofile_word, int16_t * score_matrix_word, uint8_t * dseq) {
...
#define ALIGNCORE(H, N, F, V, QR_q, R_q, QR_t, R_t)             \
...
void aligncolumns_first(__m128i * Sm, __m128i * hep, __m128i ** qp, __m128i QR_q_i, __m128i R_q_i, __m128i QR_q_r,
        ...
        __m128i M_QR_t_left, __m128i M_R_t_left, long ql) {
    ...
    for (i = 0; i < ql - 1; i++) {
        ...
        ALIGNCORE(h0, h5, f0, vp[0], QR_q_i, R_q_i, QR_t_0, R_t_0);
        ALIGNCORE(h1, h6, f1, vp[1], QR_q_i, R_q_i, QR_t_1, R_t_1);
        ALIGNCORE(h2, h7, f2, vp[2], QR_q_i, R_q_i, QR_t_2, R_t_2);
        ALIGNCORE(h3, h8, f3, vp[3], QR_q_i, R_q_i, QR_t_3, R_t_3);
    ...
    ALIGNCORE(h0, h5, f0, vp[0], QR_q_r, R_q_r, QR_t_0, R_t_0);
    ALIGNCORE(h1, h6, f1, vp[1], QR_q_r, R_q_r, QR_t_1, R_t_1);
    ALIGNCORE(h2, h7, f2, vp[2], QR_q_r, R_q_r, QR_t_2, R_t_2);
    ALIGNCORE(h3, h8, f3, vp[3], QR_q_r, R_q_r, QR_t_3, R_t_3);
...
void aligncolumns_rest(__m128i * Sm, __m128i * hep, __m128i ** qp, __m128i QR_q_i, __m128i R_q_i, __m128i QR_q_r,
        __m128i R_q_r, __m128i QR_t_0, __m128i R_t_0, __m128i QR_t_1, __m128i R_t_1, __m128i QR_t_2, __m128i R_t_2,
        __m128i QR_t_3, __m128i R_t_3, __m128i h0, __m128i h1, __m128i h2, __m128i h3, long ql) {
    __m128i h4, h5, h6, h7, h8, f0, f1, f2, f3, E, HE, HF;
    __m128i * vp;
    long i;
...
struct s16info_s {
    ...
    int16_t penalty_gap_open_target_interior;
    int16_t penalty_gap_open_query_right;
    int16_t penalty_gap_open_target_right;
    int16_t penalty_gap_extension_query_left;
    int16_t penalty_gap_extension_target_left;
    int16_t penalty_gap_extension_query_interior;
    int16_t penalty_gap_extension_target_interior;
    int16_t penalty_gap_extension_query_right;
...
void search16(s16info_s * s, unsigned int sequences, unsigned int * seqnos, int16_t * pscores,
        unsigned short * paligned, unsigned short * pmatches, unsigned short * pmismatches,
    ...
    QR_query_left = _mm_set1_epi16(s->penalty_gap_open_query_left + s->penalty_gap_extension_query_left);
            ...
            for (int c = 0; c < CHANNELS; c++) {
                for (int j = 0; j < CDEPTH; j++) {
                    if (d_begin[c] < d_end[c])
                        dseq[CHANNELS * j + c] = map_ncbi_nt16[*(d_begin[c]++)];
                    else
                        dseq[CHANNELS * j + c] = 0;
                }
                if (d_begin[c] == d_end[c])
...
//                            backtrack16(s, dbseq, dbseqlen, d_offset[c], c, paligned + cand_id, pmatches + cand_id, TODO
...
//                            strcpy(pcigar[cand_id], s->cigar);
                    ...
                    if (next_id < sequences) {
                        ...
                        while ((length == 0) && (next_id < sequences)) {
                            ...
                            address = ssa_db_get_sequence( seqno )->seq;
                            length = ssa_db_get_sequence( seqno )->seqlen;
                        ...
                        ((int16_t*) &H0)[c] = 0;
                        ((int16_t*) &H1)[c] = -s->penalty_gap_open_query_left - 1 * s->penalty_gap_extension_query_left;
                        ((int16_t*) &H2)[c] = -s->penalty_gap_open_query_left - 2 * s->penalty_gap_extension_query_left;
                        ((int16_t*) &H3)[c] = -s->penalty_gap_open_query_left - 3 * s->penalty_gap_extension_query_left;
            ...
            aligncolumns_first(S, hep, qp, QR_query_interior, R_query_interior, QR_query_right, R_query_right,
                    QR_target[0], R_target[0], QR_target[1], R_target[1], QR_target[2], R_target[2], QR_target[3],
                    R_target[3], H0, H1, H2, H3, E0, M, M_QR_target_left, M_R_target_left, qlen);
