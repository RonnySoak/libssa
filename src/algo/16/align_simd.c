...
void dprofile_dump16( uint16_t * dprofile ) {
    char * s = sym_nt_4bit;
    outf( "\ndprofile:\n" );
    for( int i = 0; i < 16; i++ ) {
        outf( "%c: ", s[i] );
        for( int k = 0; k < CDEPTH; k++ ) {
            ...
            for( int j = 0; j < CHANNELS; j++ )
                outf( " %3d", dprofile[CHANNELS * CDEPTH * i + CHANNELS * k + j] );
            outf( "]" );
        }
        outf( "\n" );
    }
}
void dumpscorematrix( uint16_t * m ) {
    for( int i = 0; i < 16; i++ ) {
        outf( "%2d %c", i, sym_nt_4bit[i] );
        for( int j = 0; j < 16; j++ )
            outf( " %2d", m[16 * i + j] );
        outf( "\n" );
    }
...
void dprofile_fill16( uint16_t * dprofile_word, uint16_t * score_matrix_word, uint8_t * dseq ) {
...
struct s16info_s {
    ...
    int maxqlen;
    int maxdlen;
    uint16_t penalty_gap_open_query_left;
    uint16_t penalty_gap_open_target_left;
    uint16_t penalty_gap_open_query_interior;
    uint16_t penalty_gap_open_target_interior;
    uint16_t penalty_gap_open_query_right;
    uint16_t penalty_gap_open_target_right;
    uint16_t penalty_gap_extension_query_left;
    uint16_t penalty_gap_extension_target_left;
    uint16_t penalty_gap_extension_query_interior;
    uint16_t penalty_gap_extension_target_interior;
    uint16_t penalty_gap_extension_query_right;
    uint16_t penalty_gap_extension_target_right;
};
...
void backtrack16( s16info_s * s, char * dseq, unsigned long dlen, unsigned long offset, unsigned long channel,
...
#if 0
    outf("Dumping backtracking array\n");
    ...
    outf("Dumping gap extension array\n");
    for(unsigned long i=0; i<qlen; i++)
    {
        for(unsigned long j=0; j<dlen; j++)
        {
...
struct s16info_s * search16_init( uint16_t score_match, uint16_t score_mismatch, uint16_t penalty_gap_open_query_left,
        uint16_t penalty_gap_open_target_left, uint16_t penalty_gap_open_query_interior, uint16_t penalty_gap_open_target_interior,
        uint16_t penalty_gap_open_query_right, uint16_t penalty_gap_open_target_right, uint16_t penalty_gap_extension_query_left,
        uint16_t penalty_gap_extension_target_left, uint16_t penalty_gap_extension_query_interior,
        uint16_t penalty_gap_extension_target_interior, uint16_t penalty_gap_extension_query_right,
        uint16_t penalty_gap_extension_target_right ) {
    ...
    for( int i = 0; i < 16; i++ )
        for( int j = 0; j < 16; j++ ) {
            ...
            if( i == j )
                ...
                value = opt_mismatch;
            ((uint16_t*) (&s->matrix))[16 * i + j] = value;
        }
    for( int i = 0; i < 16; i++ )
        for( int j = 0; j < 16; j++ ) {
            uint16_t value;
...
void search16( s16info_s * s, unsigned int sequences, unsigned int * seqnos, uint16_t * pscores, unsigned short * paligned,
        unsigned short * pmatches, unsigned short * pmismatches, unsigned short * pgaps, char ** pcigar ) {
    uint16_t ** q_start = (uint16_t**) s->qtable;
    uint16_t * dprofile = (uint16_t*) s->dprofile;
    uint16_t * hearray = (uint16_t*) s->hearray;
    unsigned long qlen = s->qlen;
    unsigned long dirbuffersize = s->maxqlen * s->maxdlen * 4;
    unsigned short * dirbuffer = s->dir;
    __m128i T, M, T0, E0;
    ...
    QR_query_interior = _mm_set1_epi16( s->penalty_gap_open_query_interior + s->penalty_gap_extension_query_interior );
    R_query_interior = _mm_set1_epi16( s->penalty_gap_extension_query_interior );
    QR_query_right = _mm_set1_epi16( s->penalty_gap_open_query_right + s->penalty_gap_extension_query_right );
    R_query_right = _mm_set1_epi16( s->penalty_gap_extension_query_right );
    QR_target_left = _mm_set1_epi16( s->penalty_gap_open_target_left + s->penalty_gap_extension_target_left );
    R_target_left = _mm_set1_epi16( s->penalty_gap_extension_target_left );
    QR_target_interior = _mm_set1_epi16(
            s->penalty_gap_open_target_interior + s->penalty_gap_extension_target_interior );
    R_target_interior = _mm_set1_epi16( s->penalty_gap_extension_target_interior );
    QR_target_right = _mm_set1_epi16( s->penalty_gap_open_target_right + s->penalty_gap_extension_target_right );
    ...
    while( 1 ) {
        if( easy ) {
            ...
            dprofile_fill16( dprofile, (uint16_t*) s->matrix, dseq );
            ...
            aligncolumns_rest( S, hep, qp, QR_query_interior, R_query_interior, QR_query_right, R_query_right,
                    QR_target[0], R_target[0], QR_target[1], R_target[1], QR_target[2], R_target[2], QR_target[3],
                    R_target[3], H0, H1, H2, H3, qlen, dir );
        }
        else {
            ...
            for( int c = 0; c < CHANNELS; c++ ) {
                if( d_begin[c] < d_end[c] ) {
                    ...
                    if( cand_id >= 0 ) {
                        ...
                        long z = (dbseqlen + 3) % 4;
                        long score = ((uint16_t*) S)[z * CHANNELS + c];
                    ...
                    if( next_id < sequences ) {
                        ...
                        ((uint16_t*) &H0)[c] = 0;
                        ((uint16_t*) &H1)[c] = -s->penalty_gap_open_query_left - 1 * s->penalty_gap_extension_query_left;
                        ((uint16_t*) &H2)[c] = -s->penalty_gap_open_query_left - 2 * s->penalty_gap_extension_query_left;
                        ((uint16_t*) &H3)[c] = -s->penalty_gap_open_query_left - 3 * s->penalty_gap_extension_query_left;
                        /* fill channel */
                        for( int j = 0; j < CDEPTH; j++ ) {
                            if( d_begin[c] < d_end[c] )
                                dseq[CHANNELS * j + c] = chrmap_4bit[*(d_begin[c]++)];
            ...
            aligncolumns_first( S, hep, qp, QR_query_interior, R_query_interior, QR_query_right, R_query_right,
                    QR_target[0], R_target[0], QR_target[1], R_target[1], QR_target[2], R_target[2], QR_target[3],
                    R_target[3], H0, H1, H2, H3, E0, M, M_QR_target_left, M_R_target_left, qlen, dir );
        }
