...
static void check_min_max( uint8_t overflow[CHANNELS_16_BIT_AVX], __m256i h_min, __m256i h_max, int16_t score_min, int16_t score_max ) {
    for( int c = 0; c < CHANNELS_16_BIT_AVX; c++ ) {
        if( !overflow[c] ) {
            int16_t h_min_array[CHANNELS_16_BIT_AVX]; // TODO align these arrays ...
