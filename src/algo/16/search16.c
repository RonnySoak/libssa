...
inline void domasked16( volatile __m128i * Sm, __m128i * hep, __m128i ** qp, __m128i * Qm, __m128i * Rm, long ql,
...
void search16_nw( uint16_t * * q_start, uint16_t gap_open_penalty, uint16_t gap_extend_penalty, uint16_t * score_matrix,
        uint16_t * dprofile, uint16_t * hearray, struct db_thread_s * dbt, long sequences, long * seqnos, long * scores,
        long * bestpos, int qlen ) {
            ...
            for( int c = 0; c < CHANNELS; c++ ) {
                if( d_pos[c] < d_end[c] ) {
                    ...
                    if( next_id < sequences ) {
                        ...
                        db_getsequence( dbt, seqno, strand, frame, &address, &length, &ntlen, c );
